...
object DisposeRabbit {
  ...
  def createStreamingContext(appName: String): StreamingContext = {
    ...
    val InputDStream: ReceiverInputDStream[String] = RabbitMQUtils.createStream(ssc, rabbitMqMap(isTest))
    var url = jdbcUrl
    if (isTest) url = jdbcUrlTest
    dispose(InputDStream, sc, spark, appName, url)
    ssc
  }

  def dispose(DStream: ReceiverInputDStream[String],
    ...
    DStream.foreachRDD(foreachFunc = rdd => {
      if (!rdd.isEmpty()) {
        ...
        val paths = rdd.mapPartitions(_.map(line => parseObject(line).getString("url"))).collect()
        try {
          val records = paths.length
          val data = sct.textFile(paths.mkString(","), records).filter(_.contains("tb_tech_student_login_record")).map(x => parseObject(x))
          ...
          val personal_development = json_name_dataARR.filter(x => (x._1 == "tb_tech_personal_development"))
            .flatMap(x => x._2)
            .map(line => parseObject(line.toString)).map(json => (
            json.getString("fStr_CourseID"),
            json.getString("fStr_StudentID"),
          ...
          var interact_count = interact_issue.map(line => (line._1, 1)).reduceByKey(_ + _)
            .map(line => (line._1, line._2 + classExamCount.getOrElse(line._1, 0))).collectAsMap()
